...
export default function WorkspaceHeader({
  ...
  return (
    <header className="h-14 border-b border-border-subtle bg-surface/90 backdrop-blur-xl px-2 sm:px-4 flex items-center justify-between z-30 shrink-0 select-none gap-2 shadow-subtle">
      {/* ── Zone: Identity (logo, room, invite) ── */}
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        >
          ...
          <span className="font-serif italic font-medium text-lg tracking-tight text-text-main hidden sm:inline">
            mindMesh
          </span>
        </a>
        ...
        <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-surface-subtle border border-border-subtle text-xs">
          ...
          <button
            ...
            className="p-0.5 text-text-muted hover:text-accent rounded transition-colors ml-0.5 disabled:opacity-50 cursor-pointer"
        ...
        <div className="hidden md:flex items-center gap-1.5">
          <div className="relative grid grid-cols-2 p-0.5 rounded-lg bg-surface-subtle border border-border-subtle text-xs shadow-subtle select-none">
            <div
              className={`absolute top-0.5 bottom-0.5 left-0.5 w-[calc(50%-2px)] rounded-md bg-surface shadow-subtle border border-border-subtle transition-transform duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] pointer-events-none will-change-transform ${
            ...
            <button
              ...
              className={`relative z-10 flex items-center justify-center gap-1.5 px-3 py-1 rounded-md text-xs transition-colors duration-200 cursor-pointer ${
          ...
          {/* Active AI Persona — compact icon chip, full text on hover */}
          {systemContext && (
            <div
              className="flex items-center justify-center w-7 h-7 rounded-lg bg-accent/10 border border-accent/20 text-accent cursor-help select-none shrink-0"
      ...
      {/* Center / Right Controls. Account/Help/Leave are pinned outside the scrollable
          region below — the exit control should never be able to scroll out of reach. */}
      <div className="flex items-center gap-2 sm:gap-3 min-w-0">
        {/* Scrollable safety net: only Live Controls + People can grow unpredictably
            (many peers, long presenter names). If they ever exceed available width,
            this scrolls instead of silently clipping. */}
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="relative flex items-center">
            {isLocalUserPresenter ? (
              ...
              >
                ...
                <span className="hidden sm:inline">Broadcasting (Stop)</span>
              ...
              >
                <Radio className={`w-3.5 h-3.5 ${isFollowing ? "text-accent animate-pulse" : "text-text-muted"}`} />
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-accent hover:bg-accent-hover text-on-accent shadow-sm transition-all duration-200 active:scale-95 group relative shrink-0 cursor-pointer"
          ...
          >
            ...
            <span className="font-medium tracking-tight hidden md:inline">
              {latestMeetingReport ? "Meeting Report" : "Commit Call"}
            </span>
            <span className="font-medium tracking-tight hidden sm:inline md:hidden">
        ...
        <div className="flex items-center -space-x-1.5 overflow-hidden shrink-0">
          ...
          {peers.length > MAX_VISIBLE_PEERS && (
            <div
              title={`${peers.length - MAX_VISIBLE_PEERS} more ${peers.length - MAX_VISIBLE_PEERS === 1 ? "collaborator" : "collaborators"}: ${peers
                .slice(MAX_VISIBLE_PEERS)
                .map((p) => p.user?.name || "Peer")
                .join(", ")}`}
              className="w-7 h-7 rounded-full border-2 border-surface flex items-center justify-center text-[10px] font-bold text-text-main bg-surface-subtle shadow-subtle cursor-default shrink-0"
            >
              +{peers.length - MAX_VISIBLE_PEERS}
            </div>
          )}
        ...
        <div className="h-4 w-px bg-border-subtle hidden sm:block shrink-0" />

        {/* ── Zone: Account (pinned — never inside the scrollable region) ── */}
        <div className="flex items-center gap-1 sm:gap-2 shrink-0">
          ...
          {/* Leave Call Button (Always Visible) */}
          <button
            type="button"
            onClick={() => setShowLeaveModal(true)}
            className="flex items-center gap-1.5 px-2.5 sm:px-3.5 py-1.5 rounded-lg text-xs font-semibold bg-rose-600 hover:bg-rose-700 text-white shadow-sm border border-rose-700/30 transition-all active:scale-95 ml-1 shrink-0 cursor-pointer"
