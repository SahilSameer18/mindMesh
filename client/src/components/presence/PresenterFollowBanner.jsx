...
export function PresenterFollowBanner({
  ...
  return (
    <div className="fixed top-16 left-1/2 -translate-x-1/2 z-50 flex flex-col items-center gap-2 pointer-events-none animate-in fade-in slide-in-from-top-3 duration-200">
      ...
      {showBanner && (
        <div className="pointer-events-auto flex items-center gap-3 px-4 py-2 rounded-full bg-slate-900/90 backdrop-blur-xl border border-violet-500/50 shadow-2xl shadow-violet-500/20 text-white text-xs font-medium">
          ...
          <button
            ...
            className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-slate-800 hover:bg-slate-700 text-slate-200 hover:text-white border border-slate-700 transition-colors text-[11px]"
          >
            <X className="w-3 h-3" />
      ...
      {showError && (
        <div className="pointer-events-auto flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/90 backdrop-blur-xl border border-amber-500/60 shadow-xl text-amber-200 text-xs font-medium animate-in fade-in duration-150">
          <AlertCircle className="w-3.5 h-3.5 text-amber-400 shrink-0" />
          <span>{contestError}</span>
          {onClearContestError && (
            <button
              onClick={onClearContestError}
              className="p-0.5 hover:bg-amber-800/40 rounded text-amber-300"
            >
              <X className="w-3 h-3" />
            </button>
          )}
