...
export default function CommitCallModal({ isOpen, onClose, canvas }) {
  ...
  return createPortal(
    <div
      className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/40 dark:bg-black/60 backdrop-blur-xl animate-in fade-in duration-200"
      onClick={!isCommitting ? onClose : undefined}
    >
      <div
        className="relative w-full max-w-4xl max-h-[92vh] flex flex-col bg-surface border border-border-subtle rounded-2xl shadow-elevated overflow-hidden glass-panel"
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-xs text-text-muted mt-0.5">
                <span className="font-mono text-text-main">Room: {roomId}</span>
                {report && (
                  <>
                    <span>•</span>
          ...
          <div className="flex items-center gap-2">
            {report && !isCommitting && (
              <button
                ...
                className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-text-main hover:bg-surface-hover bg-surface border border-border-subtle transition-colors cursor-pointer shadow-subtle"
            ...
            <button
              ...
              className="p-1.5 text-text-muted hover:text-text-main hover:bg-surface-hover rounded-lg transition-colors disabled:opacity-50 cursor-pointer"
              title="Close (Esc)"
            >
              <X className="w-5 h-5" />
            </button>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 min-h-[360px]">
          ...
          {commitError && !isCommitting && (
            <div className="mb-4 p-3 rounded-xl bg-rose-50 border border-rose-200 text-rose-800 text-xs flex items-start justify-between gap-3 animate-in fade-in duration-150 dark:bg-rose-950/40 dark:border-rose-800/60 dark:text-rose-200">
              <div className="flex items-start gap-2 min-w-0">
                <AlertTriangle className="w-4 h-4 text-rose-600 dark:text-rose-400 shrink-0 mt-0.5" />
                <div>
                  <span className="font-semibold text-rose-900 dark:text-rose-100">Synthesis Error: </span>
                  <span className="text-text-main dark:text-text-muted break-words">{commitError}</span>
                </div>
              </div>
              <button
                type="button"
                onClick={() => setCommitError?.(null)}
                className="p-0.5 text-rose-600 dark:text-rose-400 hover:text-rose-800 rounded cursor-pointer"
                title="Dismiss error"
              >
          ...
          {isCommitting ? (
            <div className="py-8 px-4 flex flex-col items-center justify-center max-w-xl mx-auto text-center space-y-6 animate-in fade-in duration-300">
              ...
              <div className="w-full space-y-3 pt-2">
                <div className="w-full h-16 rounded-xl bg-surface-subtle border border-border-subtle p-3 flex flex-col justify-between relative overflow-hidden animate-pulse">
                  <div className="w-1/3 h-3.5 bg-surface-hover rounded" />
                  <div className="w-full h-2.5 bg-surface-hover rounded" />
                  <div className="w-4/5 h-2.5 bg-surface-hover rounded" />
                </div>
                <div className="w-full h-14 rounded-xl bg-surface-subtle border border-border-subtle p-3 flex items-center justify-between relative overflow-hidden animate-pulse">
            ...
            <div className="py-6 px-2 sm:px-6 flex flex-col items-center justify-center max-w-xl mx-auto text-center space-y-5 animate-in fade-in duration-200">
              <div className="w-14 h-14 rounded-2xl bg-amber-50 border border-amber-200 dark:bg-surface-subtle dark:border-border-subtle flex items-center justify-center text-amber-600 dark:text-amber-400 shadow-sm">
              ...
              <div className="w-full text-left space-y-1.5">
                <label className="text-xs font-semibold text-text-main">Meeting Title (Optional)</label>
                <input
                  type="text"
                  value={meetingTitle}
              ...
              <div className="w-full grid grid-cols-3 gap-2.5 pt-1">
                ...
                <div className="p-3 rounded-xl bg-surface border border-border-subtle text-center shadow-subtle">
                  <div className="text-xs text-text-muted font-medium">Truth Rule</div>
                  <div className="text-xs font-bold text-emerald-600 dark:text-emerald-400 mt-1">Canvas &gt; Audio</div>
                </div>
              </div>
            ...
            <div className="space-y-4">
              {/* Tab Navigation Bar */}
              <div className="flex items-center gap-1.5 p-1 rounded-xl bg-surface-subtle border border-border-subtle overflow-x-auto scrollbar-none text-xs">
                <button
                  type="button"
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg font-semibold whitespace-nowrap transition-all cursor-pointer ${
                    activeTab === "summary"
                      ? "bg-surface text-text-main border border-border-subtle shadow-subtle"
              ...
              {activeTab === "summary" && (
                <div className="space-y-4 animate-in fade-in duration-150">
                  <div className="p-4 sm:p-5 rounded-2xl bg-surface-subtle border border-border-subtle relative">
                    ...
                    <div className="text-sm leading-relaxed text-text-main space-y-3 font-sans">
                      {report.executiveSummary ? (
                        report.executiveSummary
                          .split("\n\n")
                          .map((para, idx) => <p key={idx}>{para}</p>)
                      ) : (
                        <p className="italic text-text-muted">No executive summary available.</p>
                      )}
                    </div>
                ...
                <div className="space-y-3 animate-in fade-in duration-150">
                  {report.keyDecisions?.length > 0 ? (
                    report.keyDecisions.map((item, idx) => (
                      <div
                        key={idx}
                        className="p-4 rounded-xl bg-surface-subtle border border-emerald-500/30 hover:border-emerald-500/50 transition-colors"
                      >
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex items-start gap-2.5">
                            <span className="w-5 h-5 rounded-full bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 flex items-center justify-center text-xs font-bold shrink-0 mt-0.5">
                              {idx + 1}
                            </span>
                            <div>
                              <h4 className="text-sm font-semibold text-text-main leading-snug">
                                {item.decision}
                              </h4>
                              {item.context && (
                          ...
                          {item.owner && (
                            <span className="px-2 py-0.5 rounded text-[11px] font-medium bg-surface text-text-muted border border-border-subtle shrink-0">
                              @{item.owner}
                            </span>
                          )}
                        </div>
                  ...
                  {report.actionItems?.length > 0 ? (
                    report.actionItems.map((task, idx) => {
                      ...
                      return (
                        <div
                          key={idx}
                          className="p-3 rounded-xl bg-surface-subtle border border-border-subtle flex items-center justify-between gap-3 hover:border-border-muted transition-colors"
                        >
                          <div className="flex items-center gap-3 min-w-0">
                            <button
                              ...
                              className="text-text-muted hover:text-emerald-600 transition-colors shrink-0"
                            >
                              {isDone ? (
                            ...
                            <span
                              className={`text-sm font-medium truncate ${
                                isDone ? "line-through text-text-muted opacity-60" : "text-text-main"
                              }`}
                            >
                              {task.task}
                            </span>
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            ...
                            {task.assignee && (
                              <span className="text-xs text-text-muted font-mono bg-surface px-1.5 py-0.5 rounded border border-border-subtle">
                                @{task.assignee}
                              </span>
                            )}
                          </div>
                ...
                <div className="space-y-3 animate-in fade-in duration-150">
                  {report.unresolvedQuestions?.length > 0 ? (
                    report.unresolvedQuestions.map((q, idx) => (
                      <div
                        key={idx}
                        className="p-4 rounded-xl bg-surface-subtle border border-amber-500/30 hover:border-amber-500/50 transition-colors space-y-2"
                      >
                        <div className="flex items-start gap-2.5">
                          <HelpCircle className="w-4 h-4 text-amber-500 mt-0.5 shrink-0" />
                          <h4 className="text-sm font-semibold text-text-main">{q.question}</h4>
                        </div>

                        {q.blockerFor && (
                          <div className="text-xs text-text-muted pl-6.5 flex items-center gap-1.5">
                            <span className="text-rose-500 font-medium">Blocker:</span>
                            <span>{q.blockerFor}</span>
                          </div>
                        )}

                        {q.suggestedFollowup && (
                          <div className="text-xs text-text-muted pl-6.5 bg-surface p-2 rounded-lg border border-border-subtle flex items-start gap-1.5">
                            <span className="text-amber-600 dark:text-amber-300 font-medium shrink-0">Suggested:</span>
              ...
              {activeTab === "export" && (
                <div className="space-y-4 animate-in fade-in duration-150">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div className="p-4 rounded-2xl bg-surface-subtle border border-border-subtle flex flex-col justify-between space-y-3">
                      <div>
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-[10px] uppercase font-bold text-text-muted px-2 py-0.5 rounded bg-surface border border-border-subtle">
                            Block Kit
                          </span>
                        </div>
                      ...
                      <div className="space-y-2">
                        <input
                          ...
                          className="w-full px-2.5 py-1.5 rounded-lg bg-surface border border-border-subtle text-xs text-text-main placeholder-text-muted/50 outline-none focus:border-border-muted"
                        />
                        <button
                          type="button"
                        ...
                        <div
                          className={`p-2 rounded-lg text-xs font-medium border flex items-start gap-1.5 ${
                            exportStates.slack.simulated
                              ? "bg-amber-50 text-amber-900 border-amber-300 dark:bg-amber-950/40 dark:text-amber-200 dark:border-amber-500/50"
                              : exportStates.slack.success
                              ? "bg-emerald-50 text-emerald-900 border-emerald-300 dark:bg-emerald-950/40 dark:text-emerald-200 dark:border-emerald-500/50"
                              : "bg-rose-50 text-rose-900 border-rose-300 dark:bg-rose-950/40 dark:text-rose-200 dark:border-rose-500/50"
                          }`}
                        >
                    ...
                    <div className="p-4 rounded-2xl bg-surface-subtle border border-border-subtle flex flex-col justify-between space-y-3">
                      <div>
                        ...
                        <p className="text-xs text-text-muted mt-1">
                          Sends responsive executive briefing to attendees.
                        </p>
                      </div>
                    ...
                    <div className="p-4 rounded-2xl bg-surface-subtle border border-border-subtle flex flex-col justify-between space-y-3">
                      ...
                      <div className="space-y-2">
                        ...
                        <button
                          ...
                          className="w-full py-2 px-3 rounded-lg text-xs font-semibold bg-surface-subtle hover:bg-surface text-text-muted hover:text-text-main border border-border-subtle transition-colors flex items-center justify-center gap-1.5"
                        >
                          {copiedKey === "json" ? (
                            <Check className="w-3.5 h-3.5 text-emerald-500" />
                          ) : (
        ...
        {/* Modal Bottom Footer */}
        <div className="px-5 py-3.5 border-t border-border-subtle bg-surface-subtle flex items-center justify-between text-xs text-text-muted shrink-0">
          <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              type="button"
              onClick={onClose}
              className="px-3 py-1 rounded-lg bg-surface hover:bg-surface-subtle text-text-main border border-border-subtle transition-colors"
            >
              Close
            </button>
          </div>
