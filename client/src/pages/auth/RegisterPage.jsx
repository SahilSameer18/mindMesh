...
import { User, Mail, Lock, Eye, EyeOff, ArrowRight, ArrowLeft, AlertCircle, Sparkles, Check } from "lucide-react";
...
export default function RegisterPage() {
  const { isAuthenticated, signup, authError, setAuthError } = useAuth();
  const { navigateToDashboard, navigateToLogin, navigateToHome } = useRouter();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Error handled via useAuth / authError
    } finally {
  ...
  return (
    <div className="min-h-screen w-full flex bg-app text-text-main font-sans selection:bg-indigo-500/15 selection:text-indigo-900">
      ...
      {/* RIGHT SIDE: Registration Form (Harmonized with mindMesh current theme) */}
      <div className="w-full lg:w-[50%] xl:w-[48%] flex flex-col justify-between p-6 sm:p-10 lg:p-12 z-10 overflow-y-auto relative bg-app">
        ...
        {/* Center: Tactile Surface Card matching Landing & Dashboard */}
          ...
          <div className="bg-surface border border-border-subtle rounded-2xl sm:rounded-3xl p-6 sm:p-8 shadow-card">
            <div className="mb-6">
              ...
              <p className="mt-1.5 text-xs sm:text-sm text-text-muted leading-relaxed">
                Start mapping live team intelligence, meeting notes, and spatial graphs.
              </p>
            </div>
                  ...
                  {password && (
                    <span
                      className={`text-[11px] font-medium inline-flex items-center gap-1 ${
                        isPasswordValid ? "text-emerald-700 font-semibold" : "text-amber-700 font-medium"
                      }`}
                    >
                      {isPasswordValid ? (
                        <>
                          <Check className="w-3 h-3 text-emerald-600" /> Min 6 chars
                        </>
                      ) : (
                        "Min 6 characters"
                      )}
        ...
        <div className="text-center sm:text-left text-xs text-text-faint pt-4 border-t border-border-subtle flex flex-col sm:flex-row items-center justify-between gap-2 relative z-10">
          ...
          <span className="text-text-faint">Encrypted at rest • Scoped sessions</span>
        </div>
      </div>
    </div>
  );
