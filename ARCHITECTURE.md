...
> **Known, deliberately open items**: no per-call timeout on Groq/Gemini API requests, no `helmet` security headers, and room-integration credentials are stored as plaintext JSON rather than encrypted at rest. None of these block normal use — they're the next hardening pass, not silent gaps.
